...
func (c *Clients) SetRaiting(ctx context.Context, raiting *proto.Rating, userType string) error {
	userT := model.NewUserType(userType)
	if userT == model.User {
		_, err := c.driverClient.SetRating(ctx, raiting)
		if err != nil {
	...
	_, err := c.userClient.SetRating(ctx, raiting)
	if err != nil {
